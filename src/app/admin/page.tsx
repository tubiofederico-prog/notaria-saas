  ...
  const cols: Column<Usuario>[] = [
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-2.5">
          ...
          <div><p className="font-medium text-slate-800">{u.nombre}</p><p className="text-[11px] text-slate-500">{u.email}</p></div>
        </div>
      ),
    },
    ...
    { key: "ultimoAcceso", header: "Último acceso", render: (u) => <span className="text-slate-600">{u.ultimoAcceso}</span> },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-1.5 border-b border-slate-200">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 border-b-2 px-3.5 py-2.5 text-sm font-medium transition ${tab === t.id ? "border-brand-500 text-slate-900" : "border-transparent text-slate-600 hover:text-slate-800"}`}
          >
      ...
      {tab === "permisos" && (
        <div className="card overflow-hidden">
          <div className="border-b border-slate-200 px-5 py-4">
            <h3 className="text-sm font-semibold text-slate-900">Matriz de permisos por rol</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-white/40">
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-slate-600">Módulo</th>
                  {ROLES.map((r) => <th key={r} className="px-3 py-3 text-center text-[11px] font-medium text-slate-600">{r}</th>)}
                </tr>
              </thead>
              <tbody>
                {MODULOS.map((m) => (
                  <tr key={m} className="border-b border-slate-100 last:border-0">
                    ...
                    {ROLES.map((r) => (
                      <td key={r} className="px-3 py-3 text-center">
                        {PERMISOS_MATRIZ[r][m] ? (
                          <span className="inline-flex h-5 w-5 items-center justify-center rounded bg-emerald-500/15 text-emerald-600"><Check size={13} /></span>
                        ) : (
                          <span className="inline-flex h-5 w-5 items-center justify-center rounded bg-slate-100 text-slate-400"><X size={13} /></span>
                        )}
                      </td>
                    ))}
                  </tr>
      ...
      {tab === "notaria" && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="card p-5">
            <h3 className="mb-4 text-sm font-semibold text-slate-900">Datos de la notaría</h3>
          ...
          <div className="card p-5">
            <h3 className="mb-4 text-sm font-semibold text-slate-900">Preferencias del sistema</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={label as string} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white/40 px-3 py-2.5">
      ...
      {tab === "estados" && (
        <div className="card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-900">Estados de trámite</h3>
            <button className="btn-outline !py-1.5 text-xs" onClick={() => toast("Estado agregado", "success")}><Plus size={13} /> Agregar estado</button>
          </div>
          <div className="space-y-2">
            {ESTADOS.map((e, i) => (
              <div key={e} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white/40 px-3 py-2.5">
                <span className="flex h-6 w-6 items-center justify-center rounded bg-slate-100 text-xs font-semibold text-slate-600">{i + 1}</span>
                <span className="flex-1 text-sm text-slate-800">{e}</span>
      ...
      {tab === "tipos" && (
        <div className="card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-900">Tipos de trámite</h3>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {TIPOS_TRAMITE.map((t) => (
              <div key={t} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white/40 px-3 py-2.5">
                <Tag size={15} className="text-brand-600" />
                <span className="flex-1 text-sm text-slate-800">{t}</span>
                <button className="btn-ghost !px-2"><Edit3 size={14} /></button>
              </div>
            ))}
          </div>
...
function Toggle({ defaultOn }: { defaultOn: boolean }) {
  ...
  return (
    <button onClick={() => setOn((v) => !v)} className={`relative h-5 w-9 rounded-full transition ${on ? "bg-brand-600" : "bg-slate-200"}`}>
