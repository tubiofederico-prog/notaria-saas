...
export function Header({ onMenu }: { onMenu: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-white/[0.06] bg-ink-900/80 px-4 backdrop-blur-xl">
      ...
      <div className="flex flex-1 items-center justify-end gap-1.5 sm:flex-none">
            ...
            <div className="absolute right-0 mt-2 w-64 card p-1.5 animate-fade-in">
              {NOTARIAS.map((n) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/[0.05]"
                >
                  {n.nombre}
                  {n.id === notaria.id && <Check size={15} className="text-brand-400" />}
                </button>
              ))}
            </div>
          )}
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-80 card animate-fade-in">
              <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3">
                ...
                <span className="chip border border-brand-400/25 bg-brand-500/15 text-brand-300">{NOTIFICACIONES.length} nuevas</span>
              </div>
              <div className="max-h-80 overflow-y-auto">
                {NOTIFICACIONES.map((n) => (
                  <div key={n.id} className="flex gap-3 border-b border-white/[0.04] px-4 py-3 last:border-0 hover:bg-white/[0.03]">
                    <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${dotColor(n.tipo)}`} />
                    <div>
                      <p className="text-sm text-slate-200">{n.titulo}</p>
                      <p className="text-xs text-slate-500">{n.detalle}</p>
                      <p className="mt-0.5 text-[11px] text-slate-600">{n.hora}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-white/[0.05]"
          >
            ...
            <div className="hidden text-left md:block">
              <p className="text-xs font-medium leading-none text-white">Dra. Carla Mendoza</p>
          ...
          {userOpen && (
            <div className="absolute right-0 mt-2 w-52 card p-1.5 animate-fade-in">
              {[
                ...
                <button key={it.label} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/[0.05]">
